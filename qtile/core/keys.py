...
keys = [Key(*key) for key in [  # type: ignore
    ...
    # app launcher
    ([mod, "shift"], "r", lazy.spawn("/home/erick/.config/rofi/scripts/launcher_t1")),
    ([mod], "r", lazy.spawn("/home/erick/.config/rofi/scripts/launcher_t2")),
    
    # web browser
    ([mod], "b", lazy.spawn(cfg.browser)),

    # abri el explorador de archivos en el terminal con ranger
    ([mod, "shift"], "e", lazy.spawn("{cfg.term} -e ranger")),

    # screenshot tool
    ([], "Print", lazy.spawn("flameshot gui --delay 1000")),

    # backlight
    ([mod], "XF86AudioLowerVolume", lazy.spawn("sh -c 'xrandr --output HDMI-0 --brightness 0.4 && xrandr --output DP-5 --brightness 0.4'")),
    ([mod], "XF86AudioRaiseVolume", lazy.spawn("sh -c 'xrandr --output HDMI-0 --brightness 1.0 && xrandr --output DP-5 --brightness 1.0'")),
